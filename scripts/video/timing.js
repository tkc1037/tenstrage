...
export function calculateTiming(audioDurationSec, hook, lines, cta, options = {}) {
  const fps = options.fps ?? FPS;
  const transitionFrames = options.transitionFrames ?? 15;
  ...
  const hookFrames = Math.max(fps * 2, Math.round((hookChars / totalChars) * sequenceFrames));
  const ctaFrames = Math.max(fps * 2, Math.round((ctaChars / totalChars) * sequenceFrames));
  const infoFrames = Math.max(fps * 3, sequenceFrames - hookFrames - ctaFrames);

  let cumulativeChars = 0;
  const lineDelays = lines.map((line) => {
    const delay = Math.max(0, Math.round((cumulativeChars / infoChars) * (infoFrames - fps)) - 8);
